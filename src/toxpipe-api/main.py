...
async def create_agent(request: Request, response: Response, model: str = "azure-gpt-5-nano", temp: float = 0, max_iterations: int = 20, max_retries: int = 10, max_tokens: int=4096, max_memory_tokens: int=4096, step_timeout: float = 0, n_threads: int = 1, summarize: bool = False, seed: int = 1, reasoning_effort: str = "low", deliberation_steps: int = 5):
    # Input validation
    if temp < 0 or temp > 1:
    ...
    agent = {"agentid": str(agentid), "model": model, "temp": temp, "max_iterations": max_iterations, "max_retries":max_retries, "max_tokens":max_tokens, "max_memory_tokens":max_memory_tokens, "step_timeout":step_timeout, "n_threads":n_threads, "summarize":summarize, "seed":seed, "reasoning_effort":reasoning_effort, "deliberation_steps":deliberation_steps, "date_created":str(datetime.datetime.now())}
    ...
    if agentid not in MODEL_CACHE:
        try:
            with open(f"./created_agents/{agentid}.json", 'r') as fp:
                agent = json.load(fp)
                tpa = tp.ToxPipeAgent(name=agent["agentid"], model=agent["model"], client=client, temp=agent["temp"], max_iterations=agent["max_iterations"], max_retries=agent["max_retries"],  max_tokens=agent["max_tokens"], max_memory_tokens=agent["max_memory_tokens"], step_timeout=agent["step_timeout"], n_agents=agent["n_threads"], summarize=agent["summarize"], reasoning_effort=agent["reasoning_effort"], deliberation_steps=agent["deliberation_steps"], verbose=VERBOSE, auth=AUTH_MODE, checkpointer=checkpointer, cache=CACHE, seed=agent["seed"])
